...
const SIMPLE_SCENES = {
	SCENE_KEY.EXPLOSION: preload("res://enemy/enemy_explosion/enemy_explosion.tscn"),
	SCENE_KEY.PICKUP: preload("res://pickup/pickup.tscn")
}
...
func create_bullet(speed: float, dir: Vector2, life_span: float, key: BULLET_KEY, start_pos: Vector2):
	var new_b = BULLETS[key].instantiate()
	new_b.setup(dir, life_span, speed)
